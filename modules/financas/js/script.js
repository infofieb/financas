...
async function loadFinancialSummary(query = '') {
    ...
    const marginOutput = document.querySelector('#report-margin');
    marginOutput?.replaceChildren(marginOutput.closest('.summary-card') ? `${margin}%` : `${margin}% de margem`);
}
...
if (transactionForm) {
    ...
    const updateTransactionSummary = () => {
        ...
        const parsedValue = parseCurrency(transactionForm.elements.value.value);
        document.querySelector('#summary-type')?.replaceChildren(selectedType?.dataset.transactionType === 'income' ? 'Receita' : 'Despesa');
        document.querySelector('#summary-value')?.replaceChildren(Number.isFinite(parsedValue) ? formatCurrency(parsedValue) : 'R$ 0,00');
        document.querySelector('#summary-category')?.replaceChildren(transactionForm.elements.category.selectedOptions[0]?.text || '—');
        document.querySelector('#summary-date')?.replaceChildren(formatDate(transactionForm.elements.date.value) || '—');
        document.querySelector('#summary-account')?.replaceChildren(accountSelect.selectedOptions[0]?.text || '—');
    };
    ...
    transactionForm.addEventListener('input', updateTransactionSummary);
    transactionForm.addEventListener('change', updateTransactionSummary);
    typeOptions.forEach((option) => option.addEventListener('click', updateTransactionSummary));
    updateTransactionSummary();
